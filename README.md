...
![Image Description](./doc/yaoqiang-screenshot.png)

The [build of adhesive](_adhesive.py) is modeled as a
[BPMN process](adhesive-self.bpmn) itself, so we load it from the file
...
Implicitly if the user just clicks the OK button will send the following values
in the `context.data.publish`:
...
define a condition such as `"pypi" in context.data.publish`, or since `data` is
also available in the edge scope: `"pypi" in data.publish`.
